...
const createUser = async (req,res)=>{
    ...
    if(!validator.isValidTitle(title)){
       return res.status(400).json({status:false, msg:`title must be Mr, Mrs or Miss!`});
    }
    ...
    return res.status(201).json({status:true, msg:`User registered successfully!`, data:users});
...
const userLogin = async (req,res)=>{
    ...
    res.status(201).json({status:true, msg:`user login successful`, data:token});
